...
router.post("/", requireAuth, enforcePolicy("manage", "employees"), async (req, res) => {
  ...
  try {
    ...
    if (!employeeCode) {
      req.body.employeeCode = await generateDepartmentScopedCode({
...
router.put(
  ...
  async (req, res) => {
    ...
    const previousDepartmentName = originalEmployee.department;
    let targetDepartmentDoc = null;
    let departmentChanged = false;
    let lifecycleAuthzChanged = false;
    ...
    if (department !== undefined) {
      ...
      if (deptChanged && department) {
        targetDepartmentDoc = await Department.findOne({ name: department });
        if (!targetDepartmentDoc) {
          return res.status(404).json({ error: "Department not found" });
        }
      }
      employee.department = department;
      if (deptChanged) {
        employee.departmentId = targetDepartmentDoc?._id || null;
      }
    }
    ...
    const isReactivated =
      (previousStatus === "TERMINATED" || previousStatus === "RESIGNED") &&
      employee.status === "ACTIVE";
    const isHrToNonHrTransfer =
      departmentChanged &&
      isHrDepartmentName(previousDepartmentName) &&
      !isHrDepartmentName(employee.department);

    if (isHrToNonHrTransfer) {
      await deprovisionEmployeeAccess(employee, { forceEmployeeRole: true });
      lifecycleAuthzChanged = true;
      const departmentForCode =
        targetDepartmentDoc ||
        (employee.departmentId
          ? await Department.findById(employee.departmentId).select("_id name code")
          : null);
      if (!departmentForCode) {
        return res.status(400).json({
          error: "Cannot assign a department code without a valid target department",
        });
      }
      const autoCode = await resolveEmployeeCodeForDepartment({
        employee,
        departmentDoc: departmentForCode,
      });
      employee.employeeCode = autoCode;
    ...
    if (employee.status === "TERMINATED" || employee.status === "RESIGNED") {
      captureTerminationSnapshot(employee, {
      ...
      await deprovisionEmployeeAccess(employee, { forceEmployeeRole: true });
      lifecycleAuthzChanged = true;
      const terminationDepartmentDoc =
        originalEmployee.departmentId
          ? await Department.findById(originalEmployee.departmentId).select("_id name code")
          : originalEmployee.department
            ? await Department.findOne({ name: originalEmployee.department }).select("_id name code")
            : null;
      if (terminationDepartmentDoc && employee.employeeCode) {
        recordEmployeeCodeOwnership(employee, {
          code: employee.employeeCode,
          departmentDoc: terminationDepartmentDoc,
          at: new Date(),
        });
      }
      ...
      const restored = await restoreAssignmentFromSnapshot(employee);
      if (!restored) {
        return res.status(409).json({
          error:
            "Cannot reactivate employee: previous assignment snapshot is missing or invalid",
        });
      }
      await deprovisionEmployeeAccess(employee, { forceEmployeeRole: true });
      lifecycleAuthzChanged = true;
    ...
    await employee.save();
    if (shouldBumpAuthzVersion || lifecycleAuthzChanged) {
    ...
    if (oldEmail && employee.email && oldEmail !== employee.email) {
      try {
        ...
        // FROZEN: legacy Department.teams[] email cascade removed
        // FROZEN: legacy Department.teams[] members cascade removed
        await Team.updateMany(
...
router.post(
  ...
  async (req, res) => {
    ...
    const isHrToNonHrTransfer =
      isHrDepartmentName(fromDepartmentName) && !isHrDepartmentName(toDepartment);
    ...
    const transferRecord = {
      ...
      previousEmployeeCode:
        newEmployeeCode || isHrToNonHrTransfer ? employee.employeeCode : undefined,
    ...
    if (isHrToNonHrTransfer) {
      ...
      const autoCode = await resolveEmployeeCodeForDepartment({
        employee,
        departmentDoc: toDeptDoc,
      });
      employee.employeeCode = autoCode;
      transferRecord.newEmployeeCode = autoCode;
    }
    ...
    if (transferRecord.newEmployeeCode) {
      try {
        const backfillResult = await Attendance.updateMany(
          { employeeId: employee._id },
          { $set: { employeeCode: transferRecord.newEmployeeCode } },
        );
        if (backfillResult.modifiedCount > 0) {
          console.log(
            `[TRANSFER] Backfilled ${backfillResult.modifiedCount} attendance records: ${transferRecord.previousEmployeeCode} -> ${transferRecord.newEmployeeCode}`,
          );
        }
      } catch (err) {
    ...
    await createAuditLog({
      ...
      changes: {
        ...
        employeeCode: newEmployeeCode
          ...
          : transferRecord.newEmployeeCode
            ? { from: transferRecord.previousEmployeeCode, to: transferRecord.newEmployeeCode }
            : undefined,
      },
