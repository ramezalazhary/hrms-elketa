...
export function ManualVacationRecordsModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-900/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="relative flex max-h-[90vh] w-full max-w-2xl flex-col overflow-hidden rounded-[20px] border border-zinc-200/90 bg-white shadow-2xl ring-1 ring-zinc-950/[0.06] animate-in zoom-in-95 duration-200">
        <div className="pointer-events-none absolute -right-12 -top-12 h-40 w-40 rounded-full bg-zinc-200/40 blur-3xl" />

        <div className="relative flex items-start justify-between gap-4 p-6 md:p-8 pb-4 shrink-0 border-b border-zinc-100">
          <div className="flex gap-4 min-w-0">
            <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full border border-zinc-200/80 bg-zinc-100 text-zinc-700 shadow-sm">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1.5 text-sm text-zinc-500">
                ...
                <strong className="text-zinc-700">{employee.fullName}</strong> (not the Time off workflow).
              </p>
            </div>
          </div>
          <button
            type="button"
            onClick={onClose}
            className="flex items-center justify-center h-8 w-8 rounded-full text-zinc-400 hover:bg-zinc-100 hover:text-zinc-600 transition shrink-0 -mr-2"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="relative flex-1 overflow-y-auto px-6 md:px-8 py-4 space-y-6">
          <form
            onSubmit={handleAdd}
            className="rounded-[20px] border border-zinc-200/80 bg-zinc-50/70 p-4 ring-1 ring-zinc-950/[0.04]"
          >
            <p className="mb-3 flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-wide text-zinc-800">
              <Calendar className="h-3.5 w-3.5" />
              Add vacation / leave
            </p>
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              <label className="flex flex-col gap-1 text-xs">
                <span className="font-semibold text-zinc-600">Start</span>
                <input
                  type="date"
                  required
                  value={newStart}
                  onChange={(e) => setNewStart(e.target.value)}
                  className="rounded-xl border border-zinc-200 bg-white px-3 py-2.5 text-sm shadow-sm outline-none transition focus:border-zinc-400 focus:ring-2 focus:ring-zinc-200/80"
                />
              </label>
              <label className="flex flex-col gap-1 text-xs">
                <span className="font-semibold text-zinc-600">End</span>
                <input
                  type="date"
                  required
                  value={newEnd}
              ...
              <label className="flex flex-col gap-1 text-xs sm:col-span-2 lg:col-span-1">
                <span className="font-semibold text-zinc-600">Type</span>
                <select
                  value={newType}
            ...
            <div className="mt-4 flex flex-wrap items-center gap-2">
              ...
              {onAddAsLeaveRequest && (
                <button
                  ...
                  disabled={saving || recording}
                  className="inline-flex items-center gap-2 rounded-full border border-zinc-200/90 bg-white px-5 py-2.5 text-sm font-semibold text-zinc-800 shadow-sm transition hover:bg-zinc-50 disabled:pointer-events-none disabled:opacity-50"
                >
                  <Plus className="h-4 w-4" />
                  {recording
                    ? "Saving..."
          ...
          {onAddExcuseAsLeaveRequest && (
            ...
            >
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                <label className="flex flex-col gap-1 text-xs">
                  <span className="font-semibold text-zinc-600">Excuse date</span>
                  <input
                    type="date"
                    required
                    value={excuseDate}
                    onChange={(e) => setExcuseDate(e.target.value)}
                    className="rounded-xl border border-zinc-200 bg-white px-3 py-2.5 text-sm shadow-sm focus:border-zinc-400 focus:outline-none focus:ring-2 focus:ring-zinc-200/80"
                  />
                </label>
                <label className="flex flex-col gap-1 text-xs">
                  <span className="font-semibold text-zinc-600">Start time</span>
                  <input
                    type="time"
                    required
                ...
                <label className="flex flex-col gap-1 text-xs">
                  <span className="font-semibold text-zinc-600">End time</span>
                  <input
                    type="time"
                    required
                    value={excuseEndTime}
                  ...
                  >
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="rounded-full bg-zinc-100 px-2.5 py-0.5 text-[11px] font-semibold text-zinc-800 ring-1 ring-zinc-200/70">
                          {typeLabel}
                        </span>
                        {days != null && (
        ...
        <div className="relative flex items-center justify-end gap-3 p-6 md:px-8 pt-4 border-t border-zinc-100 shrink-0 bg-white">
          <button
            type="button"
            onClick={onClose}
