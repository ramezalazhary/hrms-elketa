...
export function SalaryIncreaseModal({ employee, orgPolicy, onClose, onSubmit }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-900/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="relative w-full max-w-lg overflow-hidden rounded-[20px] border border-zinc-200/90 bg-white p-6 shadow-2xl ring-1 ring-zinc-950/[0.06] animate-in zoom-in-95 duration-200">
        <div className="absolute right-0 top-0 -mr-16 -mt-16 h-48 w-48 rounded-full bg-zinc-300/30 blur-3xl" />
        
        <div className="relative flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-full bg-zinc-100 text-zinc-700 ring-1 ring-zinc-200/80">
              <TrendingUp className="h-5 w-5" />
            </div>
            <div>
              <h2 className="text-lg font-bold text-zinc-900 leading-none">Process Salary Increase</h2>
              <p className="mt-1 text-sm text-zinc-500">For <strong>{employee.fullName}</strong></p>
            </div>
          </div>
          <button 
            type="button" 
            onClick={onClose}
            className="flex items-center justify-center h-8 w-8 rounded-full text-zinc-400 hover:bg-zinc-100 hover:text-zinc-600 transition"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="relative flex flex-col gap-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full rounded-xl border border-zinc-200 bg-zinc-50/50 px-4 py-2.5 pl-9 text-base font-semibold shadow-sm transition focus:border-zinc-400 focus:bg-white focus:outline-none focus:ring-2 focus:ring-zinc-200/80"
                />
            ...
            {/* Effective Date */}
            <div>
              <label className="block text-xs font-bold text-zinc-500 uppercase tracking-wider mb-1.5">Effective Date</label>
              <input
                type="date"
                required
                value={date}
                onChange={(e) => setDate(e.target.value)}
                className="w-full rounded-xl border border-zinc-200 bg-zinc-50/50 px-3 py-2.5 text-sm font-medium shadow-sm transition focus:border-zinc-400 focus:bg-white focus:outline-none focus:ring-2 focus:ring-zinc-200/80"
              />
            </div>
          </div>

          {/* Real-time Calculation Display */}
          <div className="rounded-[20px] border border-zinc-200/80 bg-zinc-50/80 p-4 ring-1 ring-zinc-950/[0.04]">
            <h4 className="mb-3 flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-wide text-zinc-600">
              <Calculator className="h-3.5 w-3.5" /> Calculation Preview
            </h4>
            
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-zinc-600">Current Base Salary</span>
            ...
            <div className="mb-3 flex items-center justify-between border-b border-zinc-200/80 pb-3">
              <span className="text-sm font-medium text-zinc-600">
                Calculated increase
                <span className="ml-2 inline-flex items-center rounded-md bg-zinc-200/70 px-1.5 py-0.5 text-xs font-semibold text-zinc-800">
                  +{isNaN(activePercent) ? "0" : activePercent.toFixed(1).replace(/\.0$/, '')}%
                </span>
              </span>
              <span className="font-mono text-sm font-bold text-zinc-800">
                +{new Intl.NumberFormat("en-EG").format(isNaN(activeAmount) ? 0 : Math.round(activeAmount))} <span className="text-[10px] text-zinc-500">EGP</span>
              </span>
            </div>

            <div className="flex items-center justify-between">
              <span className="text-sm font-bold text-zinc-800">New base salary</span>
              <span className="font-mono text-lg font-semibold tracking-tight text-zinc-900">
                ...
                <span className="ml-1 text-xs font-semibold text-zinc-500">EGP</span>
              </span>
            </div>
          </div>

          <div>
            <label className="block text-xs font-bold text-zinc-500 uppercase tracking-wider mb-1.5">Reason / Notes</label>
            <textarea
              required
              rows={2}
              value={reason}
              onChange={(e) => setReason(e.target.value)}
              className="w-full resize-none rounded-xl border border-zinc-200 bg-zinc-50/50 px-3 py-2 text-sm shadow-sm transition focus:border-zinc-400 focus:bg-white focus:outline-none focus:ring-2 focus:ring-zinc-200/80"
            />
          </div>
