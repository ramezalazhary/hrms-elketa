        ...
        <div className="flex flex-wrap items-center gap-2">
          {(canAdmin || canEditVacations) && (
            <div className="relative group">
              ...
              <div className="absolute right-0 top-full z-50 mt-2 w-56 rounded-2xl border border-zinc-200/90 bg-white p-1 opacity-0 shadow-xl ring-1 ring-zinc-950/[0.06] transition-all invisible group-hover:visible group-hover:opacity-100">
                {!isTerminated && canAdmin && (
                  <button
                    type="button"
                    onClick={() => setShowTransferModal(true)}
                    className="w-full flex items-center gap-2 px-3 py-2 text-xs font-semibold text-zinc-700 hover:bg-zinc-50 rounded-lg text-left"
                  >
                    <ArrowRightLeft className="h-3.5 w-3.5 shrink-0 text-zinc-500" />
                    Transfer Department
                  </button>
                )}
                {!isTerminated && canEditVacations && (
                  <button
                    ...
                    className="w-full flex items-center gap-2 px-3 py-2 text-xs font-semibold text-zinc-700 hover:bg-zinc-50 rounded-lg text-left"
                  >
                    <Plane className="h-3.5 w-3.5 shrink-0 text-zinc-500" />
                ...
                {!isTerminated && canEditVacations && (
                  <button
                    ...
                    className="w-full flex items-center gap-2 px-3 py-2 text-xs font-semibold text-zinc-700 hover:bg-zinc-50 rounded-lg text-left"
                  >
                    <Gift className="h-3.5 w-3.5 shrink-0 text-zinc-500" />
          ...
          <Link
            className="inline-flex items-center gap-1.5 rounded-full border border-zinc-200/90 bg-white px-3 py-2 text-sm font-medium text-zinc-700 shadow-sm transition hover:bg-zinc-50"
            to="/employees"
          >
    ...
    >
      {showResetModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-900/40 backdrop-blur-sm p-4">
          <div className="w-full max-w-md rounded-[20px] bg-white p-6 shadow-xl ring-1 ring-zinc-950/[0.08]">
            <h2 className="mb-2 text-base font-semibold text-zinc-900">Reset password</h2>
      ...
      <div className="space-y-6">
        {/* Hero banner */}
        <div className="relative overflow-hidden rounded-[20px] bg-white p-6 shadow-sm ring-1 ring-zinc-950/[0.06] md:p-8">
          <div className="pointer-events-none absolute -right-10 -top-10 h-36 w-36 rounded-full bg-zinc-100/90" />
          <div className="relative flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-xl font-semibold tracking-tight text-zinc-900 md:text-2xl">{employee.fullName}</h2>
                {employee.fullNameArabic && (
                  <p className="mt-0.5 text-sm text-zinc-500 font-arabic">{employee.fullNameArabic}</p>
                )}
                <div className="mt-2 flex flex-wrap items-center gap-2 text-sm text-zinc-600">
                  <span className="inline-flex items-center gap-1.5 rounded-full border border-zinc-200/80 bg-zinc-50 px-2.5 py-0.5 text-xs font-medium text-zinc-800">
                  ...
                  {employee.nationalIdExpiryDate && (
                    <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide ring-1 ${
                      idExpiryDays <= 0 ? "bg-red-50 text-red-800 ring-red-200/80" :
                      idExpiryDays <= 30 ? "bg-amber-50 text-amber-900 ring-amber-200/80" :
                      "bg-zinc-100 text-zinc-800 ring-zinc-200/80"
                    }`}>
            ...
            <div className="rounded-xl border border-zinc-200/80 bg-zinc-50/80 px-4 py-3 text-center md:text-left">
              <p className="text-[10px] font-medium uppercase tracking-wide text-zinc-500">Department</p>
              <p className="mt-0.5 text-sm font-semibold text-zinc-900">{employee.department || '—'}</p>
              {employee.employeeCode && (
                <p className="mt-1 font-mono text-[10px] text-zinc-500">{employee.employeeCode}</p>
              )}
            </div>
          </div>
        </div>
        ...
        {isPeerView && (
          <div className="rounded-[20px] border border-zinc-200/90 bg-zinc-50/80 p-4 text-sm text-zinc-800 shadow-sm ring-1 ring-zinc-950/[0.04]">
            <p className="text-xs font-semibold uppercase tracking-wide text-zinc-700">Limited directory view</p>
        ...
        {isSelfOrAdmin && salaryIncreaseDays !== null && salaryIncreaseDays <= 30 && salaryIncreaseDays > 0 && (
          <div className="relative flex items-center gap-4 overflow-hidden rounded-xl border border-zinc-200 border-l-[6px] border-l-zinc-900 bg-white p-4 shadow-sm transition-all hover:shadow-md">
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-zinc-100 text-zinc-700 ring-1 ring-zinc-200/80">
              <TrendingUp className="h-5 w-5" />
            </div>
            <div>
              <span className="mb-1 block text-[10px] font-semibold uppercase tracking-wide text-zinc-500">Upcoming cycle</span>
              <p className="text-sm font-semibold leading-tight text-zinc-900">
                Salary increase due in <strong className="text-zinc-900">{salaryIncreaseDays} day{salaryIncreaseDays !== 1 ? "s" : ""}</strong>
        ...
        {/* Tab nav */}
        <div className="no-scrollbar flex gap-0.5 overflow-x-auto rounded-2xl bg-zinc-100/90 p-1 ring-1 ring-zinc-200/80">
          {profileTabs.map((tab) => (
            <button
              key={tab}
              type="button"
            ...
            >
              ...
              {tab === "vacations" && vacationTabBadgeCount > 0 && (
                <span className="rounded-full bg-zinc-200/80 px-1.5 py-0.5 text-[10px] font-semibold text-zinc-800 ring-1 ring-zinc-300/60">
        ...
        {activeTab === "overview" && (
          <>
            {/* Role & Workplace */}
            <div className="grid gap-4 rounded-2xl border border-zinc-200/90 bg-white p-6 shadow-sm ring-1 ring-zinc-950/[0.04] md:grid-cols-2 lg:grid-cols-3">
              <h3 className="col-span-full flex items-center gap-2 border-b border-zinc-200/80 pb-3 text-xs font-semibold uppercase tracking-wide text-zinc-800">
                <span className="h-1.5 w-1.5 rounded-full bg-zinc-700" />
                Role & workplace
              </h3>
              <p><span className="block text-xs font-semibold text-zinc-500 uppercase">Employee Code</span> <span className="text-zinc-900 font-mono bg-zinc-50 px-1.5 py-0.5 rounded border border-zinc-100">{employee.employeeCode || "N/A"}</span></p>
              <p><span className="block text-xs font-semibold text-zinc-500 uppercase">Status</span> <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${employee.status === 'ACTIVE' ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' : 'bg-amber-50 text-amber-700 border border-amber-100'}`}>{employee.status}</span></p>
              {(employee.status === "RESIGNED" || employee.status === "TERMINATED") && (employee.terminationDate || employee.terminationReason) && (
                <p className="md:col-span-2">
                  <span className="block text-xs font-semibold text-zinc-500 uppercase">Separation</span>
              ...
              <p><span className="block text-xs font-semibold text-zinc-500 uppercase">Hire Date</span> <span className="text-zinc-900">{employee.dateOfHire ? new Date(employee.dateOfHire).toLocaleDateString() : "N/A"}</span></p>
              <p><span className="block text-xs font-semibold text-zinc-500 uppercase">Next review</span> <span className="text-zinc-900">{nextReviewAt ? new Date(nextReviewAt).toLocaleDateString() : "N/A"}</span></p>
              <p><span className="block text-xs font-semibold text-zinc-500 uppercase">Job Title</span> <span className="text-zinc-900">{employee.position}</span></p>
              <p className="flex flex-col gap-1"><span className="block text-xs font-semibold text-zinc-500 uppercase">Department</span> <DepartmentBadge name={employee.department || "—"} /></p>
              <p className="flex flex-col gap-1">
                <span className="block text-xs font-semibold text-zinc-500 uppercase">Team / Unit</span>
                <div className="flex flex-wrap gap-1">
                  {teamChips.length > 0 ? (
                    teamChips.map((name, idx) => (
                      <span key={`${name}-${idx}`} className="rounded-lg border border-zinc-200/80 bg-zinc-50 px-2 py-0.5 text-xs font-semibold text-zinc-800">{name}</span>
                    ))
                  ) : (
              ...
              <p><span className="block text-xs font-semibold text-zinc-500 uppercase">Contract</span> <span className="text-zinc-900 capitalize">{employee.employmentType?.replace('_', ' ').toLowerCase()}</span></p>
              <p>
                <span className="block text-xs font-semibold text-zinc-500 uppercase">Work Location</span>
              ...
              {employee.onlineStorageLink && (
                <p className="col-span-full">
                  <span className="block text-xs font-semibold text-zinc-500 uppercase mb-1">Online Documents</span>
                  <a
                    href={employee.onlineStorageLink}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-2 rounded-lg border border-zinc-200/80 bg-zinc-50 px-3 py-1.5 text-xs font-medium text-zinc-900 transition hover:bg-zinc-100"
                  >
            ...
            <div className="grid gap-6 md:grid-cols-2">
              <div className="grid gap-4 rounded-2xl border border-zinc-200/90 bg-gradient-to-b from-white to-zinc-50/35 p-6 shadow-sm ring-1 ring-zinc-950/[0.04]">
                <h3 className="border-b border-zinc-200/80 pb-2 text-xs font-semibold uppercase tracking-wide text-zinc-900">Personal</h3>
                <p><span className="block text-xs font-semibold text-zinc-500 uppercase">Full Name (EN)</span> <span className="text-zinc-900 font-medium">{employee.fullName}</span></p>
                {employee.fullNameArabic && (
                  <p><span className="block text-xs font-semibold text-zinc-500 uppercase">Full Name (AR)</span> <span className="text-zinc-900 font-medium">{employee.fullNameArabic}</span></p>
                )}
                <p><span className="block text-xs font-semibold text-zinc-500 uppercase">Date of Birth</span> <span className="text-zinc-900">{employee.dateOfBirth ? new Date(employee.dateOfBirth).toLocaleDateString() : "N/A"}</span></p>
                <p><span className="block text-xs font-semibold text-zinc-500 uppercase">Gender</span> <span className="text-zinc-900 capitalize">{employee.gender?.toLowerCase()}</span></p>
                <p><span className="block text-xs font-semibold text-zinc-500 uppercase">Marital Status</span> <span className="text-zinc-900 capitalize">{employee.maritalStatus?.toLowerCase()}</span></p>
                <p><span className="block text-xs font-semibold text-zinc-500 uppercase">Nationality / ID</span> <span className="text-zinc-900">{employee.nationality || "N/A"} {employee.idNumber ? `(${employee.idNumber})` : ""}</span></p>
                {canAdmin && employee.nationalIdExpiryDate && (
                  <p>
                    <span className="block text-xs font-semibold text-zinc-500 uppercase">ID Expiry</span>
                    <span className={`text-sm font-medium ${idExpiryDays !== null && idExpiryDays <= 14 ? "text-red-600" : idExpiryDays !== null && idExpiryDays <= 60 ? "text-amber-600" : "text-zinc-900"}`}>
                      {new Date(employee.nationalIdExpiryDate).toLocaleDateString()}
              ...
              <div className="grid gap-4 rounded-2xl border border-zinc-200/90 bg-gradient-to-b from-white to-zinc-50/40 p-6 shadow-sm ring-1 ring-zinc-950/[0.04]">
                <h3 className="border-b border-zinc-100 pb-2 text-xs font-semibold uppercase tracking-wide text-zinc-900">Contact</h3>
                <p><span className="block text-xs font-semibold text-zinc-500 uppercase">Personal Email</span> <span className="text-zinc-900 underline decoration-zinc-300">{employee.email}</span></p>
                <p><span className="block text-xs font-semibold text-zinc-500 uppercase">Work Email</span> <span className="text-zinc-900 underline decoration-zinc-300">{employee.workEmail || "N/A"}</span></p>
                <p><span className="block text-xs font-semibold text-zinc-500 uppercase">Phone</span> <span className="text-zinc-900">{employee.phoneNumber || "N/A"}</span></p>
                {employee.emergencyPhone && (
                  <p>
                    <span className="block text-xs font-semibold text-zinc-500 uppercase">Emergency Phone</span>
                    <span className="text-zinc-900 flex items-center gap-1">
                ...
                <p><span className="block text-xs font-semibold text-zinc-500 uppercase">Address</span> <span className="text-zinc-900 whitespace-pre-line">{employee.address || "N/A"}</span></p>
                {(employee.governorate || employee.city) && (
                  <p>
                    <span className="block text-xs font-semibold text-zinc-500 uppercase">Governorate / City</span>
                    <span className="text-zinc-900 flex items-center gap-1">
                      <MapPin className="h-3 w-3 text-zinc-400" />
                ...
                {(employee.additionalContact?.whatsapp || employee.additionalContact?.skype) && (
                  <p>
                    <span className="block text-xs font-semibold text-zinc-500 uppercase">WhatsApp / Skype</span>
                    <span className="text-zinc-900 text-sm">
              ...
              <div className="grid gap-4 rounded-2xl border border-zinc-200/90 bg-gradient-to-b from-white to-zinc-50/35 p-6 shadow-sm ring-1 ring-zinc-950/[0.04]">
                <h3 className="border-b border-zinc-100 pb-2 text-xs font-semibold uppercase tracking-wide text-zinc-900">Management & reporting</h3>
                <div className="flex flex-col gap-4">
                  <div>
                    <span className="block text-xs font-semibold text-zinc-500 uppercase">Direct Manager</span>
                    <div className="mt-1 flex items-center gap-3">
                      <div className="h-8 w-8 rounded-full bg-zinc-100 flex items-center justify-center text-xs font-bold text-zinc-500 shadow-inner">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-zinc-500 font-medium">{employee.effectiveManager?.email || employee.managerId?.email || "No contact info"}</p>
                      </div>
                    </div>
                  </div>
                  <div className="h-px bg-zinc-100 w-full" />
                  <div>
                    <span className="block text-xs font-semibold text-zinc-500 uppercase">Team Leader</span>
                    <div className="mt-1 flex items-center gap-3">
                      <div className="flex h-8 w-8 items-center justify-center rounded-full bg-zinc-100 text-xs font-bold text-zinc-600 shadow-inner ring-1 ring-zinc-200/80">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] font-medium text-zinc-500">{employee.effectiveTeamLeader?.email || employee.teamLeaderId?.email || "Direct field support"}</p>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            {/* Education & Skills */}
            <div className="grid gap-4 rounded-2xl border border-zinc-200/90 bg-white p-6 shadow-sm ring-1 ring-zinc-950/[0.04]/10 md:grid-cols-3">
              <h3 className="col-span-full border-b border-zinc-100 pb-2 text-xs font-semibold uppercase tracking-wide text-zinc-900">Education & skills</h3>
              <div>
                <span className="block text-xs font-semibold text-zinc-500 uppercase mb-2">Education</span>
                {employee.education?.length > 0 ? (
                  <ul className="space-y-2">
                    {employee.education.map((edu, idx) => (
                      <li key={idx} className="text-sm bg-zinc-50 p-2 rounded-lg border border-zinc-100">
                        <span className="font-semibold text-zinc-800">{edu.degree || "—"}</span>
                        {edu.institution ? <> from <span className="text-zinc-600">{edu.institution}</span></> : null}
                        {" "}
                        ({edu.graduationDate
              ...
              <div>
                <span className="block text-xs font-semibold text-zinc-500 uppercase mb-2">Technical skills</span>
                <div className="flex flex-wrap gap-1.5">
                  {employee.skills?.technical?.length > 0 ? (
                    employee.skills.technical.map(skill => (
                      <span key={skill} className="rounded-md border border-zinc-200/80 bg-zinc-50 px-2 py-0.5 text-xs font-medium text-zinc-900">{skill}</span>
                    ))
                  ) : <span className="text-zinc-400 italic">None listed</span>}
                </div>
              </div>
              <div>
                <span className="block text-xs font-semibold text-zinc-500 uppercase mb-2">Soft skills</span>
                <div className="flex flex-wrap gap-1.5">
              ...
              <div className="col-span-full grid gap-4 md:grid-cols-2">
                <div>
                  <span className="block text-xs font-semibold text-zinc-500 uppercase mb-2">Training courses</span>
                  {employee.trainingCourses?.length > 0 ? (
                    <ul className="space-y-1 text-sm text-zinc-800">
                      {employee.trainingCourses.map((c) => (
                        <li key={c} className="rounded-lg border border-zinc-100 bg-zinc-50 px-2 py-1">{c}</li>
                      ))}
                    </ul>
                  ) : (
                ...
                <div>
                  <span className="block text-xs font-semibold text-zinc-500 uppercase mb-2">Languages</span>
                  {employee.languages?.length > 0 ? (
                    <ul className="space-y-1 text-sm text-zinc-800">
                      {employee.languages.map((row, idx) => (
                        <li key={`${row.language}-${idx}`} className="rounded-lg border border-zinc-100 bg-zinc-50 px-2 py-1">
                          {row.language}
                          {row.proficiency ? (
                            <span className="text-zinc-500"> ({row.proficiency})</span>
                          ) : null}
                        </li>
                      ))}
                    </ul>
            ...
            {/* Benefits & Financial */}
            {isSelfOrAdmin ? (
              <div className="grid gap-4 rounded-2xl border border-zinc-200/60 bg-gradient-to-br from-zinc-50/80 to-white p-6 shadow-sm">
                <div className="col-span-full flex items-center justify-between border-b border-zinc-200/50 pb-2">
                  <h3 className="text-xs font-semibold uppercase tracking-wide text-zinc-900">Benefits & financial</h3>
                  <span className="flex items-center gap-1 rounded-md bg-zinc-200/60 px-2 py-0.5 text-[10px] font-medium uppercase text-zinc-900">
                ...
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                  <div>
                    <span className="block text-xs font-semibold text-zinc-500 uppercase">Base Salary</span>
                    <span className="text-zinc-900 font-mono text-lg font-bold">
                  ...
                  <div>
                    <span className="block text-xs font-semibold text-zinc-500 uppercase">Insurance Provider</span>
                    <span className="text-zinc-900 font-medium">{employee.insurance?.provider || "N/A"}</span>
                  </div>
                  <div>
                    <span className="block text-xs font-semibold text-zinc-500 uppercase">Policy Number</span>
                    <span className="text-zinc-900 font-mono tracking-wide">{employee.insurance?.policyNumber || "N/A"}</span>
                  </div>
                  <div>
                    <span className="block text-xs font-semibold text-zinc-500 uppercase">Policy valid until</span>
                    <span className="text-zinc-900">{employee.insurance?.validUntil ? new Date(employee.insurance.validUntil).toLocaleDateString() : "N/A"}</span>
                  </div>
                  <div className="md:col-span-2">
                    <span className="block text-xs font-semibold text-zinc-500 uppercase">Medical Condition / Disease</span>
                    <span className="text-zinc-900">{employee.medicalCondition || "N/A"}</span>
                  </div>
                  <div>
                    <span className="block text-xs font-semibold text-zinc-500 uppercase">Payment Method</span>
                    <span className="text-zinc-900 font-medium capitalize">{employee.financial?.paymentMethod?.replace('_', ' ').toLowerCase() || "N/A"}</span>
                  </div>
                  <div>
                    <span className="block text-xs font-semibold text-zinc-500 uppercase">Bank Account</span>
                    <span className="text-zinc-900 font-mono tracking-wide">{employee.financial?.bankAccount || "N/A"}</span>
                  </div>
                  <div>
                    <span className="block text-xs font-semibold text-zinc-500 uppercase">Allowances</span>
                  ...
                  <div>
                    <span className="block text-xs font-semibold text-zinc-500 uppercase">Social security ref.</span>
                    <span className="text-zinc-900">{employee.financial?.socialSecurity || "N/A"}</span>
                  </div>
                </div>

                {/* Social Insurance & Medical */}
                <div className="mt-2 border-t border-zinc-200/50 pt-4">
                  <h4 className="text-xs font-semibold uppercase tracking-wide text-zinc-900 mb-3">Social Insurance & Medical</h4>
                  <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                    <div>
                      <span className="block text-xs font-semibold text-zinc-500 uppercase">Status</span>
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${employee.socialInsurance?.status === 'INSURED' ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' : 'bg-zinc-100 text-zinc-600 border border-zinc-200'}`}>
                        {employee.socialInsurance?.status === 'INSURED' ? 'Insured' : 'Not Insured'}
                      </span>
                    </div>
                    <div>
                      <span className="block text-xs font-semibold text-zinc-500 uppercase">Insurance Number</span>
                      <span className="text-zinc-900 font-mono tracking-wide">{employee.socialInsurance?.insuranceNumber || "N/A"}</span>
                    </div>
                    <div>
                      <span className="block text-xs font-semibold text-zinc-500 uppercase">Subscription Wage</span>
                      <span className="text-zinc-900 font-mono text-sm font-bold">
                    ...
                    <div>
                      <span className="block text-xs font-semibold text-zinc-500 uppercase">Insurance Date</span>
                      <span className="text-zinc-900">{employee.socialInsurance?.insuranceDate ? new Date(employee.socialInsurance.insuranceDate).toLocaleDateString() : "N/A"}</span>
                    </div>
                    <div>
                      <span className="block text-xs font-semibold text-zinc-500 uppercase">Form 6 / Expiry</span>
                      <span className="text-zinc-900">{employee.socialInsurance?.form6Date ? new Date(employee.socialInsurance.form6Date).toLocaleDateString() : "N/A"}</span>
                    </div>
                    <div>
                      <span className="block text-xs font-semibold text-zinc-500 uppercase">Basic Wage (Fixed)</span>
                      <span className="text-zinc-900 font-mono text-sm font-bold">
                    ...
                    <div>
                      <span className="block text-xs font-semibold text-zinc-500 uppercase">Comprehensive Wage (Total)</span>
                      <span className="text-zinc-900 font-mono text-sm font-bold">
                    ...
                    <div>
                      <span className="block text-xs font-semibold text-zinc-500 uppercase">Job Type (Work Type)</span>
                      <span className="text-zinc-900">{employee.socialInsurance?.jobType || "N/A"}</span>
                    </div>
                  </div>
                </div>
                ...
                {employee.insurances?.length > 0 && (
                  <div className="mt-2">
                    <span className="block text-xs font-semibold text-zinc-500 uppercase mb-2">All Insurance Records</span>
                    <div className="grid gap-2 md:grid-cols-2">
                      {employee.insurances.map((ins, idx) => (
                        <div key={idx} className="rounded-lg border border-zinc-100 bg-zinc-50/40 p-3 text-xs">
                          <p className="font-semibold text-zinc-800">{ins.providerName || "—"}</p>
                          <p className="text-zinc-500">Policy: {ins.policyNumber || "—"} · {ins.coverageType || "—"}</p>
                          {ins.expiryDate && <p className="text-zinc-500 mt-0.5">Expires: {new Date(ins.expiryDate).toLocaleDateString()}</p>}
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              <div className="grid gap-4 rounded-lg border border-zinc-200 bg-white p-6 shadow-card grayscale opacity-60 relative overflow-hidden pointer-events-none">
                ...
                <h3 className="text-sm font-bold uppercase tracking-wider text-zinc-500">Benefits & Financial Information</h3>
                <div className="grid gap-4 md:grid-cols-4">
                  <p><span className="block text-xs font-semibold text-zinc-500 uppercase">Base Salary</span> <span className="text-zinc-400">••••••</span></p>
                  <p><span className="block text-xs font-semibold text-zinc-500 uppercase">Provider</span> <span className="text-zinc-400">••••••••••</span></p>
                </div>
              </div>
            )}
          </>
        ...
        {activeTab === "documents" && (
          <div className="rounded-2xl border border-zinc-200/90 bg-white p-6 shadow-sm ring-1 ring-zinc-950/[0.04]/5">
            <div className="mb-4 flex flex-col gap-3 border-b border-zinc-100 pb-4 sm:flex-row sm:items-center sm:justify-between">
              <h3 className="text-xs font-semibold uppercase tracking-wide text-zinc-900">Required documents</h3>
              <div className="flex flex-col items-end">
                <span className="mb-1 text-[10px] font-bold uppercase tracking-widest text-zinc-800">
                   ...
                   <div key={idx} className={`flex items-start gap-3 p-3 rounded-xl border transition-all ${doc.status === "RECEIVED" ? 'bg-emerald-50/30 border-emerald-100/50' : 'bg-rose-50/30 border-rose-100/50'}`}>
                      ...
                      <div className="flex-1 min-w-0">
                         <div className="flex items-center gap-1.5 min-w-0">
                            <p className="text-xs font-bold text-zinc-800 truncate" title={doc.documentName}>{doc.documentName}</p>
                    ...
                    <li key={idx} className="relative pl-12">
                      ...
                      <div className={cardClass}>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-xs">
                          ...
                          {record.processedBy && (
                            <div className="col-span-full mt-2 border-t border-zinc-200/50 pt-2">
        ...
        {activeTab === "salary_history" && isSelfOrAdmin && (
          <div className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm">
            <div className="flex items-center justify-between mb-6">
              <div>
                <h3 className="text-lg font-bold text-zinc-800">Salary Increase History</h3>
                <p className="text-xs text-zinc-500 mt-0.5 tracking-wide">Historical timeline of base salary adjustments for {employee.fullName}</p>
              </div>
            ...
            {(employee.salaryHistory || []).length === 0 ? (
              ...
              <div className="relative">
                <div className="absolute left-5 top-0 bottom-0 w-0.5 bg-zinc-50" />
                <ol className="space-y-8">
                  {[...employee.salaryHistory].reverse().map((record, idx) => (
                    <li key={idx} className="relative pl-12">
                      ...
                      <div className="rounded-2xl border border-zinc-200/80 bg-gradient-to-br from-white to-zinc-50/25 p-5 shadow-sm transition-all hover:shadow-md">
                        <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
                          <div>
                             <span className="text-[10px] font-bold uppercase tracking-widest text-zinc-700 mb-1 block">Transaction Recorded</span>
                             <h4 className="text-sm font-bold text-zinc-900">{record.reason || "Annual Salary Increase"}</h4>
                          </div>
                          <span className="text-xs font-semibold text-zinc-500 bg-white border border-zinc-200 rounded-lg px-3 py-1 shadow-sm">
                            {new Date(record.effectiveDate).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" })}
                          </span>
                        </div>
                        
                        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                          <div className="bg-white/60 p-2.5 rounded-xl border border-zinc-100">
                             <p className="text-[9px] font-bold text-zinc-400 uppercase tracking-wider mb-1">Previous Salary</p>
                             <p className="text-sm font-semibold text-zinc-600">{new Intl.NumberFormat('en-EG', { style: 'currency', currency: employee.financial?.currency || 'EGP', maximumFractionDigits: 0 }).format(record.previousSalary)}</p>
                          </div>
                          <div className="bg-white/60 p-2.5 rounded-xl border border-zinc-200/80 ring-2 ring-zinc-950/[0.04]">
                             <p className="text-[9px] font-bold text-zinc-600 uppercase tracking-wider mb-1">New Salary</p>
                             <p className="text-sm font-bold text-zinc-800">{new Intl.NumberFormat('en-EG', { style: 'currency', currency: employee.financial?.currency || 'EGP', maximumFractionDigits: 0 }).format(record.newSalary)}</p>
                          </div>
                          <div className="bg-white/60 p-2.5 rounded-xl border border-zinc-100">
                          ...
                          {record.processedBy && (
                            <div className="col-span-full pt-2 flex items-center gap-2 border-t border-zinc-200/80/50 mt-1">
                               <div className="h-6 w-6 rounded-full bg-zinc-100 flex items-center justify-center text-[10px] uppercase font-bold text-zinc-500">{record.processedBy[0]}</div>
                               <div>
                                 <p className="text-[10px] text-zinc-400">Processed by</p>
                                 <p className="text-[10px] font-bold text-zinc-600">{record.processedBy}</p>
                               </div>
                            </div>
                          )}
                        </div>
        ...
        {activeTab === "vacations" && (
          <div className="rounded-[20px] border border-zinc-200 bg-white p-6 shadow-sm ring-1 ring-zinc-950/[0.06]">
            <div className="mb-6 flex flex-col gap-2 border-b border-zinc-100 pb-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <h3 className="text-lg font-bold text-zinc-800">Vacations & leave</h3>
            ...
            {!leaveRequestsForbidden && (
              <div className="rounded-xl border border-zinc-200/80 bg-zinc-50/30 p-4">
                <p className="text-[10px] font-bold uppercase tracking-wider text-zinc-900 mb-1">
                  Leave requests (app)
                </p>
                ...
                {leaveRequestsLoading ? (
                  ...
                  <p className="text-sm text-zinc-500 italic py-4 text-center border border-dashed border-zinc-200/80 rounded-lg bg-white/50">
                  ...
                  <ul className="space-y-2">
                    {leaveRequests.map((r) => {
                      ...
                      return (
                        <li
                          key={r._id}
                          className="rounded-lg border border-zinc-200/80/80 bg-white/90 px-3 py-2.5 sm:flex sm:items-start sm:justify-between sm:gap-3"
                        >
                          <div className="min-w-0">
                            <div className="flex flex-wrap items-center gap-2">
                              ...
                              <span className="text-[11px] font-semibold text-zinc-600">{typeLabel}</span>
                              {r.kind === "VACATION" && r.computed?.days != null && (
                                <span className="text-[11px] text-zinc-500">{r.computed.days} day(s)</span>
                              )}
                              {r.kind === "EXCUSE" && r.computed?.minutes != null && (
                                <span className="text-[11px] text-zinc-500">{r.computed.minutes} min</span>
                              )}
                            </div>
            ...
            {!leaveBalanceForbidden && (
              <div className="mt-6 rounded-xl border border-zinc-100 bg-zinc-50/20 p-4">
                <p className="text-[10px] font-bold uppercase tracking-wider text-zinc-900 mb-1">
                ...
                {leaveBalanceLoading ? (
                  ...
                  <p className="text-sm text-zinc-500 italic py-3 text-center border border-dashed border-zinc-100 rounded-lg bg-white/60">
        ...
        {activeTab === "attendance" && (
          <div className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm">
            <div className="mb-6 flex items-center justify-between">
              <div>
                <h3 className="text-lg font-bold text-zinc-800">Daily Attendance History</h3>
                <p className="text-xs text-zinc-500 mt-0.5 tracking-wide">Last 30 recorded logs for {employee.fullName}</p>
              </div>
            ...
            {isAttendanceLoading ? (
              <div className="py-20 text-center">
                 ...
                 <p className="text-sm font-medium text-zinc-500 tracking-widest uppercase">Fetching Records...</p>
              </div>
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pb-6 border-b border-zinc-100">
                   <div className="bg-zinc-50 p-3 rounded-xl border border-zinc-100">
                      <p className="text-[10px] font-bold text-zinc-400 uppercase mb-1">Present Days</p>
                      <p className="text-xl font-black text-zinc-900">{attendanceHistory.filter(h => h.status === 'PRESENT').length}</p>
                   </div>
                   ...
                   <div className="bg-zinc-50/50 p-3 rounded-xl border border-zinc-200/80/50">
                      <p className="text-[10px] font-bold text-zinc-700 uppercase mb-1">Avg. Hours</p>
                ...
                <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-zinc-50">
                    {attendanceHistory.map((log) => (
                      <tr key={log.id || log._id} className="group hover:bg-zinc-50 transition-colors">
                        <td className="py-3 text-xs font-bold text-zinc-700">
                        ...
                        <td className="py-3 text-xs font-mono font-medium text-zinc-600">
                          {log.checkIn || "—"}
                        </td>
                        <td className="py-3 text-xs font-mono font-medium text-zinc-600">
                          {log.checkOut || "—"}
                        </td>
                        <td className="py-3 text-xs font-black text-zinc-900 text-right">
      ...
      {activeTab === "assessments" && canViewAssessments && (
        <div className="rounded-2xl border border-amber-200/50 bg-white p-6 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h3 className="text-lg font-bold text-zinc-800">Performance Assessments</h3>
              <p className="text-xs text-zinc-500 mt-0.5 tracking-wide">Historical timeline of evaluations and financial bonuses for {employee.fullName}</p>
            </div>
                              ...
                              <div className="rounded-2xl border border-amber-100 bg-gradient-to-br from-white to-amber-50/20 p-5 shadow-sm transition-all hover:shadow-md">
                                <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
                                  <div>
                                    ...
                                    <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider mb-1">Overall</p>
                                ...
                                {isNewFormat && (
                                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 mb-4 text-[10px]">
                                    {[
                                      ...
                                      <div key={lbl} className="rounded-lg border border-zinc-100 bg-white/80 px-2 py-1.5">
                                        <span className="font-bold text-zinc-400 uppercase tracking-tighter block">{lbl}</span>
                                ...
                                {record.notesPrevious?.trim?.() ? (
                                  <div className="mb-3 rounded-xl border border-zinc-100 bg-zinc-50/80 px-3 py-2">
                                    <p className="text-[9px] font-bold uppercase tracking-widest text-zinc-400 mb-1">Notes / previous</p>
                                    <p className="text-xs text-zinc-600 whitespace-pre-wrap">{record.notesPrevious}</p>
                                  </div>
                                ) : null}

                                <div className="bg-white/60 p-4 rounded-xl border border-zinc-100 mb-4">
                                  <p className="text-[9px] font-bold uppercase tracking-widest text-zinc-400 mb-1">Feedback</p>
                                  <p className="text-sm text-zinc-700 italic">"{record.feedback}"</p>
                                </div>

                                <div className="flex flex-wrap items-center justify-between gap-4 border-t border-amber-100/50 pt-3">
                                  <div className="flex items-center gap-2">
                                    <div className="h-6 w-6 rounded-full bg-zinc-100 flex items-center justify-center text-[10px] uppercase font-bold text-zinc-500">
                                      {record.evaluatorId?.fullName?.[0] || "?"}
                                    </div>
                                    <div>
                                      <p className="text-[10px] text-zinc-400">Evaluated by</p>
                                      <p className="text-[10px] font-bold text-zinc-600">{record.evaluatorId?.fullName || "A Manager"} ({record.evaluatorId?.position || "Management"})</p>
                                    </div>
                                  </div>
                                  {record.getThebounes && (
